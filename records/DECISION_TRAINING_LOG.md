...
| Gate I | 在 **TS0(1,305) / ArchiveII(3,966) / bpRNA-new(5,401)** 上评测：F1(micro+macro) / INF / ECE / Brier / NLL / 非法结构率 / 延迟分桶 | **进行中**。**as-trained（`w=-1`，无任何选择）两个 split 已完成**：TS0 **0.4953**（macro 0.4858，gap_rc 0.0002）、ArchiveII **0.5834**（macro 0.6503，gap_rc 0.0044，3,950 行）。bpRNA-new 的 as-trained 正在跑。`w=0.75` 对照：TS0 0.4959、ArchiveII 0.5829。**聚合口径下我们低于** ViennaRNA centroid 0.5393 / 0.6207 与 MXfold2 0.5651，**但分层后方向不同**（§14.21 来源、§14.22 长度）。step 6000/10000 趋势与 2×2 评测排队中 |
| **长度 × 来源 二维格子**（新，本轮） | §14.22 的长度桶未匹配来源、§14.21 的来源分层只在 ≤100 nt 做，**两个轴没有分离** | **待做**。需要 `tools/length_bucketed_leaderboard.py` 与 `tools/stratify_by_source.py` 的笛卡尔积（CRW×{≤100,100–200,200–400,>400}、RFAM×同）。在做出这个格子之前，**不得**把长度效应或来源效应单独归因 |
| **按来源分层报告**（新，本轮） | TS0/VL0 的聚合 F1 必须与 **CRW / RFAM 分层**数字同时报，否则误导（§14.21） | 已测：TS0 CRW≤100 **0.9664** vs centroid 0.6729；TS0 RFAM≤100 0.5490 vs 0.6209。口径已写入 `spec/benchmark_decision.md` §4.2b；工具 `tools/stratify_by_source.py`（**闭集来源词表**，bpRNA-new 正确报"分层不可得"）。**待办**：对 ArchiveII 做同样的来源分层（其名字非 `bpRNA_*` → 目前报不可得，需外部映射表） |
