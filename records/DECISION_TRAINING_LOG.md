...
| split | n | 我们（`w=-1`，as-trained） | 我们（`w=0.75`） | ViennaRNA centroid | ViennaRNA mfe | MXfold2 | 我们自己的 Nussinov+Turner |
|---|---|---|---|---|---|---|---|
| TS0 | 1,288 | **0.4953** | 0.4959 | **0.5393** | 0.5222 | **0.5651** | 0.2124 |
| ArchiveII（3,950/3,966） | 3,950 | 待出（排队中） | **0.5829** | 0.6207 | 0.5764 | — | 0.2010 |
| bpRNA-new | 5,388 | 待出（排队中） | **0.3094** | **0.6770** | 0.6379 | — | **0.3015** |
...
| Gate I | 在 **TS0(1,305) / ArchiveII(3,966) / bpRNA-new(5,401)** 上评测：F1(micro+macro) / INF / ECE / Brier / NLL / 非法结构率 / 延迟分桶 | **进行中**。TS0 headline（`w=-1`，as-trained）**micro F1 = 0.4953** / macro 0.4858 / P 0.5325 / R 0.4629（§14.20）；`w=0.75` 时 0.4959。仍**低于** ViennaRNA centroid **0.5393** 与 MXfold2 **0.5651**。ArchiveII 只能用 **3,950/3,966** 行（16 条 >1024 nt 超出 RiNALMo `max_pos`，见 §14.17）；as-trained 的 ArchiveII / bpRNA-new 与 step 6000/10000 趋势**排队中**。**分层结果见 §14.21：TS0 CRW≤100 我们 0.9664 vs centroid 0.6729** |
| **按来源分层报告**（新，本轮） | TS0/VL0 的聚合 F1 必须与 **CRW / RFAM 分层**数字同时报，否则误导（§14.21） | 已测：TS0 CRW≤100 **0.9664** vs centroid 0.6729；TS0 RFAM≤100 0.5490 vs 0.6209。**待办**：把分层口径写进 `spec/benchmark_decision.md` §4，并对 ArchiveII / bpRNA-new 做同样的来源分层（bpRNA-new 无来源标签 → 只能按长度/家族近似，须显式说明） |
...
| C1-c | 免 DP 校准判据 `|ECE(System-1) − ECE(exact)| ≤ 0.02` | **分两口径，不得混写**：① **裸头**（训练时直接用头的 sigmoid）在 step 3500、`w=-1` 下 gap = **0.1348**（`w=0.75` 时 0.1053），**未过**，但从 step 20 的 0.549 单调下降；② **免 DP 仿射重标定**（2 个参数，在 VL0 上拟合，评测期不跑配分函数）后 gap = **0.0002**（`w=-1`）/ **0.0006**（`w=0.75`）/ **0.0093**（ArchiveII，`w=0.75`），**均 PASS**。当前可主张的只有 ② |
