...
## §14.70 例行监控（12:25）：静默期零新事件、统计零漂移 60/60 维持；RiNALMo-ft 下载速率坍缩至 ~2.4KB/s——deadline 风险首次亮红（2026-09-26 12:25）

> 节号说明：用户指令模板仍写"§14.61 起"，实际盘上已至 §14.69（§14.66
> 已声明此偏差，惯例沿用）；本节取 §14.70。四臂关键判定（bigtr1_s1
> 交互为负 2-seed 复现、big_s3 容量 4-seed 统计、ff_tr1_s1 TR1
> 2-seed、bigsum 0.6302≥0.63 混杂终判闭环）与 RNA-FM 骨干轴
> negative 均已在 §14.63–14.65/§14.68 定稿。本轮为出数后静默期
> 例行确认轮：例行项全部零漂移，唯一实质变化在第三节（下载）。
...
`pgrep -af train_decision` 空 = 预期行为（§14.66 判定顺序再次实证：
先看日志终点步数 + ledger completed，再看进程——"完成"不是"消失"）。
恢复协议零触发。

### 二、评测与统计：零新事件 + 零漂移确认

- §14.69 收盘（10:19）后 `eval_decision/` 无新 ow_* 目录（find
  -newermt 10:19 为空）；watch3–6 进程全退，串行评测协议无并发；
- alerts_decision.log：**2026-09-26 当日零新增告警行**（全 202 条
  均为 09-25 ff_tr1_b4_s0 stale 历史告警，早已结案）；mtime 12:17
  再被触碰、内容零变化——§14.69 起的"触碰无内容"模式持续第二轮，
  无数据影响，继续观察；
- `tools/stats_definitive.py` 重跑：headline 与 §14.68/14.69 完全
  一致（RNA-FM 70.5%/61.7% ratio、20-test 家族、Holm 6.4e-146/
  1.4e-198，stats_definitive.json 重落盘 byte-stable），
  `tools/check_draft_v34.py` **60/60 PASS**。draft 维持 v3.10，
  本轮零漂移、无版本 bump、无新数字可写（禁手抄原则下的"无操作"
  即正确操作）。

### 三、RiNALMo-ft 下载：速率坍缩（本轮唯一实质变化，风险亮红）

- launchd `com.user.rinalmo-dl` running（curl pid 62767，09:14 起
  state 正常）；12:19 实测 1,107,693,564/2,604,809,354 = **42.5%**；
- 外层轮次实证：round1 11:10 = 1,090,295,252 → round2 11:42 =
  1,098,102,676 → round3 12:15 = 1,102,929,916；每轮（~32 min
  外层重连周期）净增 8.0/7.8/4.8 MB 且**逐轮收窄**——Zenodo 每次
  error-18 断流后重连只拿到极小增量即再断；
- 有效速率实测：09:14→12:19（185 min）净增 ~26MB ≈ **2.4KB/s**
  （10:23→12:19 窗口 ~3.7KB/s）。剩余 1.497GB 按 2.4–3.7KB/s 需
  **4.7–7.3 天**，将越过 09-30 deadline——**§14.69 的诚实 ETA
  区间（12:30–20:00）作废**，本节起按实测速率重估；
- 机制照旧健康：curl `--retry 5 --retry-all-errors` 内部吸收
  error-18（Range 重请求语义，无数据损坏风险）、外层 500 轮循环 +
  `--max-time 3600` 每小时强制重开保证不死循环——**问题不在健壮性
  在带宽**（Zenodo 侧限速/拥塞，非本端可控）；
- 影响面如实界定：§4.3d quoted→measured 闭环是 deadline 前唯一
  剩余决策面，但 draft v3.10 当前按 quoted 处理已合规（60/60
  PASS），**下载失败不阻塞投稿**，只损失一个加分对照项；
- 下轮决策选项（本轮不擅动）：A) 维持等待（限速解除不可预测）；
  B) 排查备用源/镜像（如 Zenodo API 直链、社区 mirror）；C) 放弃
  本轮闭环，draft 维持 quoted 现状收尾。

### 四、GPU 与排队实验（维持无新臂）

共享卡 0–5 实测（12:25，used/40GB，util）：

| 卡 | used | util | 判定 |
|---|---|---|---|
| 0 | 26.6 | 99% | 他人满载 |
| 1 | 19.2 | 31% | 他人占（§14.69 后加重，10.7→19.2） |
| 2 | 10.4 | 0% | **30.5GB 空闲、无本组任务** |
| 3 | 23.5 | 100% | 他人满载 |
| 4 | 5.9 | 14% | **34.5GB 空闲、无本组任务** |
| 5 | 11.0 | 0% | **~30GB 空闲、无本组任务**（10:19 后他人任务退出） |

卡 2/4/5 满足">15GB 空闲且无我们任务"的认领条件，但**维持无新臂**：
五臂已出数、骨干轴 negative 定稿（§14.68"backbone axis closed,
no follow-up arms"），当前排期为零（等出数后决策已兑现为无新臂）。
nvidia-smi 上 cunyuliu 名下 418MiB 常驻进程属 `rna_sc.watch_all`
（另一项目的监控进程），非本组任务。卡 6/7 无本组任务。

### 五、仓库与副本状态

- 本轮唯一入库变更：`records/DECISION_TRAINING_LOG.md` 本节；
- 历史遗留 untracked（`logs/`、`scripts/run_s1_casc_eval.sh`、
  `spec/spec.md.bak.14.59`）沿用 §14.66–69 惯例维持不入库；
- commit + push（GitHub Cunyu-Liu/JEPA-mRNA）后，paper/spec/
  records scp 拉回本地 `~/rna-jepa-sync`。

### 六、待办（下轮 ~14:30 或下载剧变时）

- [ ] 下载速率决策（§14.70 第三节选项 A/B/C）——若 14:30 实测仍
      <10KB/s，建议走 B（查备用源）或 C（quoted 收尾）
- [ ] 常规巡查：零告警确认（alerts mtime 触碰模式第三轮观察）、
      五臂终态无回退、无新 ow_*
...
## §14.72 例行监控（16:19–16:50）：r2d 中程健康 + §14.71 卡位勘误（实跑共享卡 1，非卡 6）+ 并行会话 v3.9/v3.11 已入两 commit 未推——本轮代推送、统计零漂移 60/60（2026-09-26 16:50）

> 节号说明：沿用 §14.66 的偏差声明，用户模板"§14.61 起"对应盘上
> §14.72。四臂关键判定（bigtr1_s1 交互为负 2-seed 复现、big_s3 容量
> 4-seed、ff_tr1_s1 TR1 2-seed、bigsum ≥0.63 混杂终判）与 RNA-FM 轴
> 均已在 §14.63–14.65/§14.67 定稿，本轮无追加 seed、无四查项。

### 一、训练臂与进程复核（零回退、恢复协议零触发）

四个原监控臂终态 20000/20000 与 §14.71 表逐项一致（bigtr1_s1 19:17/
big_s3 20:33/ff_tr1_s1 20:32/bigsum 22:58，09-25）；唯一在训新臂
r2d（rinalmo_r2d_b4_s0）16:32–16:37 实测 **step 7875→8100/20000**，
train_log 新鲜（mtime 16:32，5 分钟内），进程 3587129 存活（ps 实测
14:38:27 启动、elapsed 2h00m）。

**r2d 中程读数（供出数对照，与 ff 系不可直比 loss）**：loss 66→38
下行中（step 8100 实测 38.26，terms.nll 38.86 与 bigsum 系同族口径
——未除以序列长度的原始 nll）；grad_norm 46→258 区间波动，无 FATAL。
watch3–6 全退、queue_decision 空、串行评测协议无并发（watch6 收盘
08:52 后 eval 无并发痕迹）。

**§14.71 卡位勘误（本轮唯一实质修正）**：r2d 进程 environ 实测
`CUDA_VISIBLE_DEVICES=1`，nvidia-smi compute-apps 证实 PID 3587129
占 **共享卡 1（31.7GB）**；§14.71 所记"卡 6（22.8GB）"有误——卡 6
现仅 3.9GB 他人残留进程，本组无任务。r2d 估速修正：14:38:27 起 8100
步 / 118 分钟 ≈ 1.15 st/s，剩余 11900 步 ≈ 2.9h，**完训 ETA ~19:25**
（§14.71 按 0.8 st/s 估 21:30 偏保守）。

### 二、评测与统计：零新事件 + 零漂移确认

- `ls -t eval_decision/ | head -6` 前排的三个非 ow_ JSON
  （rinalmo_ft_official_ts0 14:51 / rinalmo_ft_bprna_new 15:01 /
  structrfm_bprna_new 15:11）为并行会话产物，已记账于 §14.67
  （14.67 表值与本轮直读 json 复核一致：RiNALMo-ft TS0 strict micro
  0.7210 / new 0.4489；structRFM new 0.5438），本轮无新 ow_* 目录
  （最后一笔 09-26 08:52 watch6 收盘）。
- `tools/stats_definitive.py` 重跑两遍 md5 一致
  （5284307e…，byte-stable，无漂移）；`tools/check_draft_v34.py`
  **60/60 PASS**；draft 由并行会话推进至 **v3.11**（commit 0d3466d，
  15:27，含 §4.3g 三项分解 + 重跑基线行 + 18/18 文本检查）；禁手抄
  原则下本轮不动 draft 统计行。
- alerts_decision.log：当日零新增（mtime 触碰模式延续，内容零变化）。

### 三、GPU 与排队实验（维持无新臂）

共享卡 0–5（16:19 实测）：卡 0 21.9GB@100%、卡 1 37.6GB@0%仪表（本
组 r2d 占 31.7GB）、卡 2 31.0GB@79%、卡 3 29.2GB@100%、卡 4 28.4GB@
100%、卡 5 22.5GB@93%——无一张卡同时满足">15GB 空闲且无本组任务"
（卡 0/5 各有 ~19GB 名义空位但满载他人任务）。维持无新臂（用户排期：
等 r2d 出数后再议；并行会话 Plan-B 判定规则 +0.03 over ff）。

### 四、仓库状态：v3.9/v3.11 两 commit 待推——本轮代推送

`git status -sb`：`main...origin/main [ahead 2]`（f094d28 v3.9 +
0d3466d v3.11，并行会话 15:31 提交后未 push），工作树干净。本轮入库
本节 + 代推 origin/main；并行会话 WIP（tools/eval_*.py 等 untracked
与 decision_head.py 修改）维持不入库，留出数后正式提交。

### 五、本轮入库与下轮待办

- 本轮入库变更：`records/DECISION_TRAINING_LOG.md` 本节；push 后 scp
  拉回 paper/spec/records 至本地 `~/rna-jepa-sync`。
- 下轮（~18:30 或 r2d 完训 ~19:25 时）：
  - [ ] r2d 尾程监控；完训后手跑 evaluate_decision.py 出双 split
        result.json（模板串行协议，注意 r2d 复用 ff_b4_s0 的数据/
        种子/嵌入路径，双 split 逐项对齐后按 §4.3 scorer 消融行记账）
  - [ ] structRFM Mathews 宽容口径（dbn 落盘为前提）+ draft 对比表
        扩行（若并行会话未先做）
