...
| RFAM | 200–400 | 126 | 0.4265 | 待测 | 待测 | 待测 | — |
| RFAM | >400 | 15 | 0.2766 | 待测 | 待测 | 待测 | — |

#### 结论：来源决定胜负方向，长度只决定幅度

1. **在 CRW 上，两个长度桶我们都大幅领先**（+0.295 / +0.272）。
2. **在 RFAM 上，两个长度桶我们都落后**（−0.061 / −0.108）。
3. → **与物理基线的胜负由「来源」决定，与「长度」无关。**
   长度只在**同一来源内部**改变幅度（RFAM 内部 0.5598 → 0.4272 → 0.4265 → 0.2766，
   单调下降）。**§14.22 里"差距随长度扩大"的说法需要修正**：
...
- **CRW 的 100–200 桶只有 16 条**，该格子的 +0.272 证据很弱；可信的是 ≤100 桶（n=68）
  与 VL0 上的独立复现（n=81，0.9709 vs 0.6702）。
- `RFAM` 的 200–400（n=126）与 >400（n=15）两格**基线尚未跑**，所以"RFAM 上我们始终落后"
  目前只在 ≤200 nt 上被证明。
