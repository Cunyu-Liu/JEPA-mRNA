...
→ 全 split 上先验权重带来 **+0.040**（与 VL0 的 +0.048 一致）。**当前距 ViennaRNA centroid 0.5393 差 0.044，
距 MXfold2 0.5651 差 0.069。** 注意这两点之间步数与权重同时变了，**不能当作纯权重效应**；
纯权重效应由 VL0 的受控扫描给出（同一 checkpoint）。
