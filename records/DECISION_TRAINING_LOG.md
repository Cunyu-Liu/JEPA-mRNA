...
| split | 我们的模型 micro F1 | ViennaRNA centroid | ViennaRNA mfe | **Nussinov+Turner**（我们自己的先验） |
|---|---|---|---|---|
| **TS0**（in-distribution） | 0.4959 | 0.5393 | 0.5222 | 0.2124 |
| **bpRNA-new**（跨家族 OOD） | **0.3094** | **0.6770** | 0.6379 | **0.3015** |
| ArchiveII | 评测中 | 0.6207 | 0.5764 | 0.2010 |

**两条关键读数**：

1. **我们的模型从 TS0 的 0.4959 掉到 bpRNA-new 的 0.3094（−0.187）；而 ViennaRNA centroid
   从 0.5393 升到 0.6770（+0.138）** —— 物理模型的跨家族表现**变好**，学习模型**变差**。
   "OOD 衰减更小"这个主张**被数据否定**。
2. **在 bpRNA-new 上我们（0.3094）几乎等于自己的 Nussinov+Turner 先验（0.3015）**，
   即**训练出来的判别力在跨家族时基本消失**，退化回"没有学到东西"的起点。
