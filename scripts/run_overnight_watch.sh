#!/bin/bash
# Overnight watcher: as arms reach step-20000 snapshots, evaluate them
# (TS0; bprna_new for the big/capacity arms too). Serial protocol.
set -u
export PYTHONPATH=/mnt/cunyuliu/pylibs:/home/cunyuliu/rna-jepa/src
export TMPDIR=/mnt/cunyuliu/tmp
export OMP_NUM_THREADS=2
PY=/home/cunyuliu/miniconda3/envs/lucaone/bin/python
REPO=/home/cunyuliu/rna-jepa
ART=/mnt/cunyuliu/rna-jepa
DEV=$ART/ss_data/jsonl/bprna_vl0.jsonl
LOG=$ART/eval_decision/overnight_watch.log
EVAL_GPU="${RNAJEV_EVAL_GPU:-MIG-27707c52-3cf5-55f8-858f-1419d97bbdf3}"
MARK=$ART/eval_decision/.overnight_done

wait_quiet() {
  for _ in $(seq 1 1440); do
      pgrep -f "[e]valuate_decision.py" >/dev/null && { sleep 30; continue; }
      pgrep -f "[r]un_tr1_final_eval.sh" >/dev/null && { sleep 30; continue; }
      pgrep -f "[r]un_fromscratch_final_eval.sh" >/dev/null && { sleep 30; continue; }
      break
  done
}

eval_one() {
  local arm="$1" step="$2" split="$3"
  local ckpt="$ART/ckpts/${arm}_step${step}.pt"
  local out="$ART/eval_decision/ow_${arm}_step${step}_${split}"
  [ -f "$out/result.json" ] && return 0
  [ -f "$ckpt" ] || return 1
  wait_quiet
  echo "=== OW $arm step=$step $split $(date +%T) ==="
  env CUDA_VISIBLE_DEVICES="$EVAL_GPU" \
      PYTHONPATH="$PYTHONPATH" TMPDIR="$TMPDIR" OMP_NUM_THREADS=2 nice -n 15 \
      "$PY" "$REPO/eval/ss/evaluate_decision.py" \
      --checkpoint "$ckpt" \
      --data "$ART/ss_data/jsonl/${split}.jsonl" \
      --embedding-split "$split" \
      --calib-data "$DEV" \
      --prior-weight -1 \
      --out "$out" \
      --encoder-size 35M --device cuda --head-chunk 8 \
      --tag "ow_${arm}_step${step}_${split}"
  echo "=== OW $arm $split done rc=$? ==="
}

{
  echo "overnight watch start $(date +%T)"
  for _ in $(seq 1 720); do
      eval_one rinalmo_ff_b4_s3 20000 bprna_ts0
      eval_one rinalmo_ff_b4_s4 20000 bprna_ts0
      eval_one rinalmo_ff_b4_s5 20000 bprna_ts0
      eval_one rinalmo_cascR_b4_s0 20000 bprna_ts0
      eval_one rinalmo_big_b4_s0 20000 bprna_new
      eval_one rinalmo_bigtr1_b4_s0 20000 bprna_ts0
      eval_one rinalmo_bigtr1_b4_s0 20000 bprna_new
      eval_one rinalmo_ff_tr1_b4_s0 40000 bprna_ts0
      eval_one rinalmo_ff_tr1_b4_s0 40000 bprna_new
      n=$(ls "$MARK".* 2>/dev/null | wc -l)
      all_done=1
      for spec in "rinalmo_ff_b4_s3" "rinalmo_ff_b4_s4" "rinalmo_ff_b4_s5" "rinalmo_cascR_b4_s0"; do
          [ -f "$ART/eval_decision/ow_${spec}_step20000_bprna_ts0/result.json" ] || all_done=0
      done
      [ -f "$ART/eval_decision/ow_rinalmo_big_b4_s0_step20000_bprna_new/result.json" ] || all_done=0
      [ "$all_done" = "1" ] && { echo "all watch targets done $(date +%T)"; break; }
      sleep 600
  done
} >> "$LOG" 2>&1
echo detached
